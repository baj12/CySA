...
plotScatterBJ <- function(x, chs, gate = NULL, color_by = NULL, facet_by = NULL, bins = 100,
  ...
  df <- data.frame(t(as.matrix(y)), cd,
    check.names = FALSE,
  ...
  if (is.null(color_by)) {
    ...
    if (ncol(x) < 1000) {
      ...
      scales <- scale_fill_gradientn(trans = "sqrt", colors = c(
        "navy",
    ...
    if (is.numeric(df[[color_by]])) {
      guides <- NULL
      scales <- scale_color_gradientn(colors = c(
        "navy",
        rev(brewer.pal(11, "Spectral"))
      ))
    } else {
      ...
      guides <- guides(col = guide_legend(override.aes = list(
        alpha = 1,
        size = 3
      )))
    }
  ...
  ggplot(df, aes(
    x = .data[[chs[1]]], y = .data[[chs[2]]],
    col = switch(is.null(color_by) + 1,
      .data[[color_by]],
      NULL
    )
    # col = NULL,
